...
function fonk(){
	if (y2.getAttribute("durum")==="pasif"){
		...
		let alt=[];
		let nan=[];
		// 'canc.','1:0 pen.','abn.','award.'
		// && scores[i].innerHTML.split(':')[1].substring(1)==null  --  && scores[i].children.length==0
		for (i=0; i<scores.length;i++){
			if(scores[i].innerHTML!='award.' && scores[i].innerHTML!='abn.' && scores[i].innerHTML!='canc.' && scores[i].innerHTML!='postp.' && scores[i].innerText.split(':')[1].length<3 && scores[i].children.length==0){
